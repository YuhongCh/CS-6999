...
numParInCell = ti.field(dtype=int, shape=numCell)
...
def neighborSearch():
    ...
    for par in range(numPar):
        cell = int(getCell(position[par]))  # get cell ID from position
    ...
    for i in range(numPar):
        ...
        for ii in ti.static(range(9)):
            cellToCheck = int(neiCellList[ii])
            # which cell to check
