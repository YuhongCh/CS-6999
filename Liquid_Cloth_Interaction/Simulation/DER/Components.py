...
from Liquid_Cloth_Interaction.Simulation.SimulationUtils import p_find_normal
...
class DER_Vertices:
    ...
    def __init__(self, num_vertices: int):
        ...
        self.fixed = ti.ndarray(dtype=bool, shape=num_vertices)
...
class DER_CurvatureBinormal:
    ...
    def __init__(self, edges: DER_Edges):
        ...
        self.kb = ti.ndarray(dtype=tm.vec3, shape=num_edges-1)
        self.edges = copy(edges)

    @ti.func
    def t_update(self):
        num_edges = self.edges.t_get_num_edges()
        for idx in ti.ndrange((1, num_edges)):
            i = idx[0]
...
class DER_ReferenceFrame:
    ...
    def __init__(self, edges: DER_Edges):
        num_edges = edges.p_get_num_edges()
        self.ref_frame1 = ti.ndarray(dtype=tm.vec3, shape=num_edges)
        self.ref_frame2 = ti.ndarray(dtype=tm.vec3, shape=num_edges)
        self.ref_twist = ti.ndarray(dtype=float, shape=num_edges)
        self.edges = copy(edges)

    @ti.func
    def t_set_start_frame(self, rf1: tm.vec3):
        """ Set up the initial reference frame if there is any """
        # determine the initial reference frame
        num_edges = self.edges.t_get_num_edges()
        if (rf1.norm() > Epsilon) and (tm.dot(rf1, self.edges.dir[0]) > Epsilon):
            rf1.normalized()
        else:
            rf1 = p_find_normal(self.edges.dir[0])
        self.ref_frame1[0] = rf1
        self.ref_frame2[0] = tm.cross(self.edges.dir[0], rf1)
...
class DER_MaterialFrame:
    ...
    def __init__(self, ref_frame: DER_ReferenceFrame):
        ...
        self.mat_frame1 = ti.ndarray(dtype=tm.vec3, shape=num_edges)
        self.mat_frame2 = ti.ndarray(dtype=tm.vec3, shape=num_edges)
        self.ref_frame = copy(ref_frame)

    @ti.func
    def t_update(self):
        num_edges = self.ref_frame.edges.p_get_num_edges()
        for i in ti.ndrange(num_edges):
...
class DER_Curvature:
    ...
    def __init__(self, kb: DER_CurvatureBinormal, mat_frame: DER_MaterialFrame):
        ...
        self.kappa = ti.ndarray(dtype=tm.vec4, shape=num_edges-1)
        self.kb = copy(kb)
        self.mat_frame = copy(mat_frame)

    @ti.func
    def t_update(self):
        num_edges = self.kb.edges.p_get_num_edges()
...
class DER_GradCurvature:
    ...
    def __init__(self, edges: DER_Edges, mat_frame: DER_MaterialFrame, curvature: DER_Curvature):
        num_edges = edges.p_get_num_edges()
        self.grad_kappa = ti.MatrixNdarray(11, 4, shape=num_edges-1, dtype=float)
        self.edges = copy(edges)
        self.mat_frame = copy(mat_frame)
    ...
    def t_update(self):
        ...
        num_edges = self.edges.p_get_num_edges()
        for idx in ti.ndrange((1, num_edges)):
            i = idx[0]
            ...
            if chi < Epsilon:
                print("Warning: chi value way too small!")
                chi = Epsilon
